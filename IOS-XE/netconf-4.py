...
with manager.connect(host=router["host"], port=router["port"], username=router["username"], password=router["password"], hostkey_verify=False) as m:
    ...
    print('Connected')
    interface_netconf = m.get(netconf_filter) # this is an xml response so we need to convert it to a python dict
...
name = interface_python['interfaces']['interface']['name']['#text'] # in order to get raw text, you have to specify #text
print(name)
